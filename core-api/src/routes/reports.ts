...
router.post("/", authenticate, validate(createReportSchema), async (req: Request, res: Response, next: NextFunction) => {
  ...
  const report = await Report.create({
    ...req.body,
    reporterPhone: req.user!.phoneNumber || req.body.reporterPhone || "9834538742",
    confirmationCode,
    status: "pending",
  });
...
router.patch("/:id/confirm", authenticate, async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (req.user!.role !== "admin" && report.reporterPhone !== req.user!.phoneNumber && report.reporterPhone !== "9834538742") {
